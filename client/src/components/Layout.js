...
const Layout = ({ children }) => {
  ...
  const getMenuItems = () => {
    ...
    if (user?.role_name.toLowerCase()=== "receptionist" || user?.role_name.toLowerCase() === "admin") {
      baseItems.push(
        {
          key: "/patients",
          icon: <UserOutlined />,
    ...
    if (user?.role_name.toLowerCase() === "doctor" || user?.role_name.toLowerCase() === "admin") {
      const medicalChildren = [];

      if (user?.role_name.toLowerCase() === "doctor") {
  ...
  return (
    <AntLayout style={{ minHeight: "100vh" }}>
      ...
      >
        ...
        >
          <span style={{ fontWeight: 600, fontSize: 18, color: "#ffffffff" }}>
            {collapsed ? (
              <img
                src={logo}
                alt="Logo"
