...
const CreateReceiptPage = () => {
  ...
  const [noteOverview, setNote] = useState("");
  const [submitType, setSubmitType] = useState("confirmed");
  ...
  const updateMedicine = useCallback((id, updatedFields) => {
    console.log("Update gi dị", updatedFields);
    setMedicineList((prev) =>
  ...
  const handleMedicineCodeChange = useCallback(
    (rowId, selectedMedicineId) => {
      ...
      const medicineCode =
        ...
        selectedMedicineId;
      const unitName =
        selectedMedicine.unit_name || selectedMedicine.unit || "";
  ...
  const updateQuantityMedicine = async () => {
    try {
      for (const item of medicineList) {
        // 1. Lấy thông tin thuốc hiện tại
        ...
        console.log("So Luong thuoc hien tai: ", newQuantity);
        ...
        console.log("Trang thai new cua thuoc: ", newStatus);
  ...
  const handleSubmit = useCallback(
    async (values, type) => {
      ...
      try {
        ...
        console.log("Payload DK receipt: ", payload);
    ...
    [medicineList, getMedicineInfo, user, navigate, noteOverview]
  );
        ...
        >
          {medicines.map((medicine) => (
            <Option
              ...
              value={medicine.id || medicine.medicine_id}
            >
...

    {
      ...
      render: (_, record) => (
        <Input
          ...
          onChange={(e) => {
            console.log("Note of Medicine: ", e.target.value);
            updateMedicine(record.id, { note_batch: e.target.value });
          }}
          placeholder="Nhập ghi chú bổ sung cho phiếu nhập (tối đa 150 ký tự)"
        />
      ),
    },
    {
  ...
  return (
    <div style={{ padding: "5px", background: "#f0f2f5", minHeight: "100vh" }}>
      ...
      <Form
        ...
        layout="vertical"
        onFinish={(values) => {
          // Lưu ý: onFinish sẽ chỉ trigger khi validation pass
          // Nhưng ta cần biết user nhấn button nào (draft hay confirmed)
          // => Dùng state để track
          handleSubmit(values, submitType);
        }}
      ...
      >
        ...
        <Card
          className="card-note-receipt mb-6"
          title="Ghi chú"
          style={{
            marginBottom: 16,
            borderRadius: 8,
        ...
        >
          <TextArea
             placeholder="Nhập ghi chú bổ sung cho phiếu nhập (tối đa 300 ký tự)"
            maxLength={300}
            rows={4} // số dòng hiển thị
            value={noteOverview} // state lưu giá trị
            onChange={(e) => {
              console.log("Set note qua hay:", e.target.value);
          ...
          <Row justify="end">
            <Space size="middle">
              ...
              <Button
                ...
                loading={loading}
                onClick={() => {
                  // Không cần validate khi lưu nháp
                  handleSubmit(form.getFieldsValue(), "draft");
                }}
                className="text-[#0050b3]"
                icon={<SaveOutlined />}
                style={{
                  borderRadius: 8,
                  minWidth: 150,
              ...
              {/* BUTTON LƯU PHIẾU NHẬP (CÓ VALIDATE) */}
              <Button
                type="primary"
                size="large"
                htmlType="submit" // ← QUAN TRỌNG: trigger form validation
                loading={loading}
                icon={<SaveOutlined />}
                onClick={() => setSubmitType("confirmed")} // Set type trước khi submit
                style={{
                  borderRadius: 8,
                  minWidth: 150,
                  background:
      ...
      <MedicineSearchModal
        ...
        onSelect={(medicine) => {
          console.log("Thuốc đã chọn", medicine);
          ...
          setMedicineList((prev) => {
            // nếu list đang rỗng → thêm luôn
            if (prev.length === 0) return [newItem];

            const lastItem = prev[prev.length - 1];

            // Kiểm tra lastItem.medicine_id có rỗng không
            const isLastEmpty =
